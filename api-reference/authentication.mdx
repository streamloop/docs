...
The **Try it** playground on each REST endpoint page lets you authenticate with either an **API key** (the `X-API-Key` header) or a **bearer token** (your OAuth access token). Pick whichever you have, fill in the value, and the request runs against the live API.
